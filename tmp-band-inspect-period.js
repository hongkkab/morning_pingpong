    ...
    const data = await ev(send, `(() => {
      ...
      const inputs = [...document.querySelectorAll('input')].map((el, i) => {
        ...
        return {
          ...
          parent: clean(el.parentElement && (el.parentElement.innerText || el.parentElement.textContent || '')).slice(0, 180)
        };
      });
      const buttons = [...document.querySelectorAll('button, a')].filter(visible).map((el, i) => {
        const r = el.getBoundingClientRect();
        return { i, tag: el.tagName, text: clean(el.innerText || el.textContent || ''), href: el.href || '', cls: el.className || '', rect: { x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), h: Math.round(r.height) } };
      }).filter(x => x.text || x.href).slice(0, 120);
      return { url: location.href, title: document.title, inputs, buttons, body: clean(document.body.innerText || '').slice(0, 3000) };
    })()`);
    fs.writeFileSync(path.join(outDir, 'period-ui-inspect.json'), JSON.stringify(data, null, 2), 'utf8');
