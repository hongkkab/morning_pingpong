...
(async()=>{
 ...
 run(function(){
   ...
   if(lg!=='morning'&&shareResultText(resultRoute)!=='')throw Error('Other league shared to Band');
   const bm=publicResultMatches().find(x=>lgOf(x)==='morning');
   const text=shareResultText({player:bm.aId,league:'morning',round:rdOf(bm)}),u=new URL(text.split('\n').pop()),r=readResultRoute(u.href);
   if(r.player!==bm.aId||r.league!=='morning'||r.round!==rdOf(bm)||u.searchParams.get('src')!=='band')throw Error('Share round-trip');
