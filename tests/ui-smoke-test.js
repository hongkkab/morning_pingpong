    ...
    (() => {
      ...
      try {
        ...
        const ok1 = h.includes('조 순위') && !h.includes('A조 순위')
          && h.includes('class="brkbd') && h.includes('하위 4강') && h.includes('brkp win') && h.includes('cupres')
          && !h.includes('data-clg=');
        ...
        const ok2 = h2.includes('brkp win') && h2.includes(nameOf(ids[0])) && h2.includes('최종 순위');
        return ok1 && ok2;
      } finally {
        S.matches = oldMatches;
