...
async function setRange(send, start, end) {
  ...
  if (!ok.ok) throw new Error(`set range failed: ${JSON.stringify(ok)}`);
  await sleep(4500);
}
...
async function collect(send) {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    previous = sig;
    if (stable >= 5) break;
    await ev(send, `window.scrollBy(0, Math.max(1200, window.innerHeight * 1.45))`);
    await sleep(650);
  }
  return [...new Map(snapshots.flatMap(s => s.rows).map(r => [r.post, r])).values()];
}
...
(async () => {
  ...
  try {
    ...
    const start = process.argv[2] || '2026. 08. 14.';
    const end = process.argv[3] || '2026. 08. 15.';
    await setRange(send, start, end);
    const rows = await collect(send);
    const morning = rows.filter(r => /모닝즐탁|모닝리그|금요모닝|토요모닝|일요모닝|모닝/.test(r.text));
    const label = `${start}_${end}`.replace(/[^\d]+/g, '_').replace(/^_|_$/g, '');
    fs.writeFileSync(path.join(outDir, `period_${label}.json`), JSON.stringify({ start, end, rows, morning }, null, 2), 'utf8');
    console.log('rows', rows.length, 'morning', morning.length);
    for (const r of morning) console.log(`${r.post}\t${r.href}\t${r.text.slice(0, 900)}`);
  } finally {
    ws.close();
  }
})().catch(e => {
