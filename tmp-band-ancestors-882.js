  ...
  try {
    ...
    const data = await ev(send, `(() => {
      ...
      for (let i = 0; i < 16 && node; i++, node = node.parentElement) {
        ...
        arr.push({ depth: i, tag: node.tagName, cls: node.className || '', id: node.id || '', textLen: clean(node.innerText || node.textContent || '').length, text: clean(node.innerText || node.textContent || '').slice(0, 500), imgs: node.querySelectorAll ? node.querySelectorAll('img').length : 0, buttons: node.querySelectorAll ? node.querySelectorAll('button').length : 0, rect: { x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), h: Math.round(r.height) } });
      }
      return { url: location.href, link: link ? { text: clean(link.innerText || link.textContent || ''), href: link.href } : null, ancestors: arr };
    })()`);
    fs.writeFileSync(path.join(outDir, 'ancestors-926087882.json'), JSON.stringify(data, null, 2), 'utf8');
